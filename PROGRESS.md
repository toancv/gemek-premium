...
- **NEXT: H4 — FE both apps drop `gemek_refresh` localStorage: authStore + client interceptor switch to cookie-implicit refresh (`withCredentials: true`, send `X-Requested-With` on /auth/refresh, stop reading/writing localStorage refresh, login keeps access-token-in-memory). Then H5 (CTO auth smoke: login / refresh-after-expiry / logout / re-login, BOTH apps, dev profile http). Close-out after H5: remove legacy body channel (login-response refreshToken field + body-param refresh path). After sprint: (c) staff user-mgmt DESIGN SESSION, (d) move-out admin UI, N2 (unblocked), N4, TEMP_HIDDEN_DEFERRED, VN user guide.
