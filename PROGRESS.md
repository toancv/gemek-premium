...
- **NEXT: N3 P3 — ticket lifecycle dispatch (C1–C6 per design §C): create→admins (new UserRepository role query), assign VN-ize + subscription rows, status-change thread dispatch, rating events. TERMINOLOGY: DONE = «Hoàn tất».** Also pending: CTO smoke-test N1 (resident :81). Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
