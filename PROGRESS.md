...
**RESUME POINTER (one line):** **C2.3b P2 DONE — committed (`3ab5001`), awaiting CTO :80 smoke — NEXT = P3 (COVER banner in preview + polish).** P2 = FE-only media manager on `/announcements/:id/edit` (DRAFT only): unified grid lists all uploaded images (thumbnails from the detail manifest), upload control with a "Đặt làm bìa" toggle (off=INLINE / on=COVER, kind-at-upload; cover-replace is BE in-tx), per-item "Chèn vào bài" (INLINE only — drops `![mô tả ảnh](announcement-media:{id})` at the cursor via the reused `insertMarkdown` ref+RAF) + "Xoá" (VN confirm dialog), COVER item shows a BÌA badge + Xoá only (no insert). **Inline preview render FOLDED INTO P2** (P2/P3 boundary moved): the preview pane now receives the draft manifest (`id`→`mediaId` map, mirrors resident `AnnouncementDetailPage.tsx:55`) so inserted INLINE placeholders resolve live after upload/insert; **COVER-as-banner in preview stays P3.** New FE hooks `useUploadAnnouncementMedia` (multipart file+kind, returns media id) + `useDeleteAnnouncementMedia`, both refetch `['announcements',id]` (`refetchType:'all'`) so grid+preview refresh. 3 new VN error strings added (`ANNOUNCEMENT_MEDIA_TYPE_NOT_ALLOWED`/`_LIMIT_EXCEEDED`/`ANNOUNCEMENT_NOT_DRAFT`) + 413-size special-case. **No BE/contract change** (existing C2.2 endpoints; API-SPEC unchanged). admin tsc+vite build green; `@gemek/ui` vitest 32/32 (admin has NO vitest harness → no admin unit tests, per P1). `/code-review` (high, workflow): 0 Must-fix; applied 5 FE-fixable (cover-toggle reset, insert-at-0 append-when-unfocused, mediaManifest useMemo, 413 message, test-guard extend); debts logged below. **NEXT = P3** (one line): COVER banner in the admin preview + polish (optimistic insert, shared Modal, media-type DRY). Below = P1/P1.5 history.
...
| C2.3b P2 | media manager (upload/cover-toggle/insert/delete) on /:id/edit + inline preview render (manifest `id`→`mediaId`, folded from P3) | **DONE — awaiting CTO :80 smoke** | `3ab5001` |
| C2.3b P3 | COVER-as-banner in the admin preview + polish (optimistic insert, shared Modal reuse, media-type DRY) | PENDING | — |
| C3 | file attachments (non-image) | PENDING | — |

**OPEN ITEMS / BACKLOG (priority order):**
1. **C2.3b P3 — COVER banner in admin preview + polish** (NEXT) — P2 DONE (`3ab5001`): media manager (upload/cover-toggle/insert/delete) on `/:id/edit` + inline preview render (manifest `id`→`mediaId`, folded from P3), tsc+build green, awaiting CTO :80 smoke. P3 remaining: render the COVER manifest entry as a **banner** above the preview body (inline images already resolve); optional polish from the P2 /code-review debts (optimistic insert after upload so the new image appears before refetch; reuse `@gemek/ui` `Modal` for the delete-confirm; collapse `AnnouncementMediaItem` vs `AnnouncementMediaManifestEntry` duplication via a shared `toMediaManifest` helper). Plan: `reports/c2-3b-authoring-ux-investigation.md` + DECISIONS 2026-06-26 C2.3b P2 entry.
