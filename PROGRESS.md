...
- **NEXT: full N3 browser smoke-test (CTO, ports 80/81 — covers N1 resident bell too), then P9 docs (API-SPEC: follow endpoints + viewer flags isFollowing/redacted + visibility param + new notification types/events; PUT→POST fix spec:2008/2018; spec:1069 annotation; backlog file update).** Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
