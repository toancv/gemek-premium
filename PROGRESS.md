...
- **NEXT: small-fix turn — backlog (a) submitter display «{fullName} - {unitNumber}» in non-redacted ticket views + (b) assign-form appointment-date mm/dd→dd/MM (likely native input type=date → VNDatePicker swap, check first).** Then larger backlog per CTO priority: (c) staff user-management module (DESIGN SESSION — multi-role schema question), (d) move-out admin UI (BE endpoint exists, ResidentController:172), N2 rich announcements, N4 ticket media+comments (F-05-gated), TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20.
