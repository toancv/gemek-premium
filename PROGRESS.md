...
1. **Smoke C2.3a** — manual BROWSER XSS smoke per `reports/c2-3a-announcement-image-render.md`. Fixture seeds end-to-end, logins work (helper resets picked residents to `Smoke@1234`, both probe `200`). **BLOCKED on presign public-URL:** the manifest `media[].url` host is the internal `http://minio:9000` (single `MinioClient` used for both put + presign — `MinioConfig.java:53`, `FileStorageService.java:89`), unreachable from the browser, and port 9000 is not host-published → **positive image render fails (broken `<img>`)**; out-of-scope *visual* check also blocked. SignedHeaders=host means no client/nginx host-rewrite (would 403) — fix must be server-side (mint presign against a browser-reachable host). Pre-existing C2.1/C2.2 gap. Options + recommendation (Option A: `MINIO_PUBLIC_ENDPOINT` + presign-only client): `reports/minio-presign-public-url-investigation.md` — AWAITING CTO ruling. **Still doable now (no image needed):** the inertness DOM/Network checks (external-url / `javascript:` / `data:` / unknown-id render nothing) and the **link-form inert anchor** (already verified inert, expected). Latest fixture: announcement `dde9ebc7-…`, detail `http://localhost:81/announcements/dde9ebc7-ea30-4935-9b89-029a2ffc2b5a`, IN `0909616883` / OUT `0922333111` (both `Smoke@1234`, :81 incognito).
