...
1. **Smoke C2.3a** — manual BROWSER XSS smoke (5 storable attacks + positive render + scope/leak) per `reports/c2-3a-announcement-image-render.md`. Fixture seeds end-to-end AND **logins now work** — the helper resets each picked resident's password to `Smoke@1234` via `PUT /api/users/{id}/reset-password` (admin), and both printed logins probe `200`+JWT. **AWAITING the CTO browser smoke; nothing blocks it.** Helper has a localhost-only safety guard (it mutates passwords). Latest run: announcement `dde9ebc7-ea30-4935-9b89-029a2ffc2b5a`, detail `http://localhost:81/announcements/dde9ebc7-ea30-4935-9b89-029a2ffc2b5a` (resident portal :81, `RequireAuth`). IN-SCOPE `0909616883` / `Smoke@1234` (sees images); OUT-OF-SCOPE `0922333111` / `Smoke@1234` (empty manifest). **Use a clean/incognito session on :81 logged in as the in-scope resident.** Re-run the helper any time for a fresh fixture + freshly-reset creds.
