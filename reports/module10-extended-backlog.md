...
- **(e) Self-service profile page for ALL admin-portal roles (ADMIN/BOARD_MEMBER/TECHNICIAN)** — view own profile, change own phone/email, change own password (with current-password verification). **Investigation done 2026-06-18, see `reports/e-self-profile-investigation.md`.** Verdict: NOT FE-only — me-read (`GET /api/auth/me`) and self change-password (`PUT /api/auth/me/password`, verifies old pw + complexity) ALREADY EXIST and are live; only self profile-UPDATE (phone/email/fullName) is MISSING → needs a small gated BE phase (new `PUT /api/auth/me/profile`, authenticated-only, principal-derived id, phone/email uniqueness own-row-excluded) then the FE profile page. Token survives phone/email change (subject = user UUID, phone claim informational). Open CTO product call: whether email/phone change needs a confirmation step. NOT started.
- **(d) Resident move-out flow — ✅ DONE (2026-06-22, FE only; pending CTO :80 smoke).** BE endpoint already existed (`POST /api/residents/{id}/move-out`, ResidentController:172). Admin UI added to `ResidentsPage` (per-row, no detail page exists): "Kết thúc cư trú" button → confirm modal with `VNDatePicker` (default today, editable, Option B) + optional notes + irreversibility copy; «Đã chuyển đi · dd/MM/yyyy» badge for moved-out residents (button hidden). Reports: `reports/d-moveout-build.md`, `reports/d-moveout-ui.md`. Soft end-of-residency (sets moveOutDate, clears primary-contact, NO login block), not a hard delete.
- **(f) (OPTIONAL hardening — NOT scheduled, NOT a security fix) FE identity re-adoption on bootstrap.** Both authStores (admin + resident) re-adopt whatever identity the shared host-scoped `refreshToken` cookie resolves to on bootstrap, with **no binding between the in-memory access-token subject and the displayed `user`** — and a brief window where a 401-interceptor refresh swaps the token but keeps the old `user` label (`client.ts:55`). Surfaced by the 2026-06-18 identity-confusion investigation (`reports/c-identity-confusion-diagnosis.md`; DECISIONS entry "Cross-session identity confusion — investigated, NOT a defect"). The server boundary is already correct (every request honored per its own token); this is **UX/robustness only**. Optional enhancement: on bootstrap/refresh compare the refreshed access-token subject to the held `user.id`, and on mismatch force an explicit re-login and/or surface "session changed in another tab". **Do NOT prioritize over feature/doc work** — not a security defect, only a shared-browser-jar footgun.
