...
- **NEXT: CTO browser smoke-test (FINAL Module 10 step).** Rebuild `docker compose up -d --build nginx`; admin :80, resident :81. NOT browser-verified: bell badge increment on publish, panel list, News expand + border clear, mark-all-read no-405. Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
