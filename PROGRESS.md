...
- **⏸ BACKLOG (c) staff user-mgmt — IN PROGRESS. P0 DONE (CTO rulings, commit 20ed802); P1 DONE (2026-06-17).** Rulings: `DECISIONS.md` "Backlog (c) … CTO rulings (2026-06-17)". Evidence: `reports/c-staff-usermgmt-investigation.md`. P1 notes: `reports/c-p1-userspage.md`. Phase plan (order FIXED): P0 docs → **P1 UsersPage (done)** → P2 RequireRole audit on ALL admin pages → P3 admit TECHNICIAN (tickets-only) → P4 audit_logs [split] → P5 docs.
  ...
  - **Resume pointer:** P1 + P1b + P2.5 + **P2.6 (BE overdue filter)** complete. P2.6 awaiting CTO docker redeploy to confirm live `total=459` on :80 (folds into the pending P2.5 :80 smoke). **NEXT after CTO go = P2.7** (FE: SLA-breached card on TicketsPage via `useTicketCount({overdue:true})` + stat→`?overdue=true` drill-down needing `useSearchParams`) — DO NOT start without CTO go. P2 (RequireRole audit, dashboard ruling = Option 2 per `reports/c-p2-route-audit.md`) and P3 (admit TECHNICIAN) still pending in fixed order; Do NOT start P3 until P2 lands.
