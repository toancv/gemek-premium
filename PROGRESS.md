...
1. **Smoke C2.3a** — manual BROWSER XSS smoke (5 storable attacks + positive render + scope/leak) per `reports/c2-3a-announcement-image-render.md`. Fixture seeds clean end-to-end (helper fixed + `gemek` bucket created). **BLOCKED on resident login:** data survived the rebuild (user/announcement/media all present), login field is phone (correct), but the auto-picked in-scope resident `0909616883` has **no `Demo@1234` password** — that password is valid only for the admin `0901100001`, NOT for the imported residents in this DB (probe → 401). Recovery (CTO ruling): reset a chosen in-scope resident's password via `PUT /api/users/{id}/reset-password`, or enhance the smoke to use a known-password resident. Diagnosis: `reports/c2-2-media-upload-500-diagnosis.md` → "Resident login failure after rebuild".
