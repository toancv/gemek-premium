...
- **⏸ BACKLOG (c) staff user-mgmt — IN PROGRESS. P0 DONE (CTO rulings, commit 20ed802); P1 DONE (2026-06-17).** Rulings: `DECISIONS.md` "Backlog (c) … CTO rulings (2026-06-17)". Evidence: `reports/c-staff-usermgmt-investigation.md`. P1 notes: `reports/c-p1-userspage.md`. Phase plan (order FIXED): P0 docs → **P1 UsersPage (done)** → P2 RequireRole audit on ALL admin pages → P3 admit TECHNICIAN (tickets-only) → P4 audit_logs [split] → P5 docs.
  ...
  - **(c) ORDER NOW RECONCILED: P1✓ P1b✓ P2.5✓ P2.6✓ P2.7✓ P2-STEP-B✓ → P3 UNBLOCKED.** Next = P3 (admit TECHNICIAN: widen authStore `ALLOWED_ROLES` to `[ADMIN,BOARD_MEMBER,TECHNICIAN]` at both gates — one-liner, now safe because every non-/tickets admin route guards the role out). Then P4 (audit_logs, split) + P5 (docs). **Awaiting CTO go for P3** (+ CTO :80 smoke covering both the P2.6/2.7 SLA bundle AND the P3 technician login once P3 lands).
