...
- **NEXT: N3 P6 — SLA scheduler (V16 marker columns `sla_warning_notified_at`/`sla_overdue_notified_at`, 15-min cron, fixed 2h warning per G2, direct-invocation tests) + ContractExpiryScheduler marker fix (G6), per design §D.** Also pending: CTO smoke-test N1 (resident :81). Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
