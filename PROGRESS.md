...
- **NEXT: H3 — BE httpOnly refresh cookie (E2=Option 1, DECISIONS hardening entry): Set-Cookie on /auth/login + /auth/refresh (profile-driven Secure, SameSite=Strict, Path=/api/auth), refresh reads cookie with body-param fallback KEPT, logout clears cookie, CORS exact-origins + allowCredentials, X-Requested-With on refresh. Then H4 (FE both apps drop gemek_refresh) → H5 (CTO auth smoke both apps). After sprint: (c) staff user-mgmt DESIGN SESSION, (d) move-out admin UI, N2 rich announcements (unblocked), N4, TEMP_HIDDEN_DEFERRED, VN user guide.
