...
- **NEXT: N3 P4 — household notify on `ResidentServiceImpl.createResident` (C9, `HOUSEHOLD_MEMBER_ADDED`, reuses `findActiveByApartmentId`), per design §C/§F.** Also pending: CTO smoke-test N1 (resident :81). Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
