...
- **NEXT: N3 P7 — FE resident (per design §F): `NOTIF_ROUTES` += `Ticket: id => /tickets/{id}`; create-form public toggle «Công khai phản ánh để cư dân khác theo dõi»; «Cộng đồng» tab (`?visibility=community`) + follow/unfollow button; vi.ts keys; tsc + vite green.** Then P8 (admin bell clickable) + P9 (docs: API-SPEC new endpoints, PUT→POST fix spec:2008/2018, spec:1069 annotation). Also pending: CTO smoke-test N1 (resident :81). Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
