...
- **NEXT: H2 — `announcements/` key-prefix routing in `assertPresignAccess` (prefix → per-surface rule; unit tests: ticket prefix keeps household rule, announcement prefix any-authenticated, unknown prefix 403). Then H3 (BE httpOnly refresh cookie, body-param fallback kept) → H4 (FE both apps drop gemek_refresh) → H5 (CTO auth smoke both apps). After sprint: (c) staff user-mgmt DESIGN SESSION, (d) move-out admin UI, N2 rich announcements (now unblocked), N4, TEMP_HIDDEN_DEFERRED, VN user guide.
