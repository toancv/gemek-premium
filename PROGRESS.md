...
- **NEXT: N3 P8 — FE admin bell clickable (port resident `handleNotifClick` + admin `NOTIF_ROUTES` Ticket → admin ticket detail route — verify actual admin route in admin App.tsx). Then P9 docs (API-SPEC: follow endpoints + viewer flags + visibility param; PUT→POST fix spec:2008/2018; spec:1069 annotation).** Full N3 browser smoke-test happens AFTER P8 (one pass covers the loop). Also pending: CTO smoke-test N1 (resident :81). Then remaining backlog: TEMP_HIDDEN_DEFERRED, VN user guide, hardening F-04/F-05/SEC-20 (deferred).
