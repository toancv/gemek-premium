...
- **NEXT: H5 — CTO auth browser smoke (login / refresh-after-expiry / logout / re-login, BOTH apps, dev profile http, TWO browser profiles per cookie-collision note). Rebuild nginx first: `docker compose up -d --build nginx`. Close-out after H5: remove legacy body channel (login-response refreshToken field + body-param refresh path). After sprint: (c) staff user-mgmt DESIGN SESSION, (d) move-out admin UI, N2 (unblocked), N4, TEMP_HIDDEN_DEFERRED, VN user guide.
